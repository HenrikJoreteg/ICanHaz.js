...
*/
function a(){var e=this,d={cache:{},partials:{}};this.VERSION="0.7";this.addTemplate=function(f,g){if(this[f]){throw"Can't add a template with reserved name: "+f+"."}else{if(d.cache.hasOwnProperty(f)){throw"You've already got a template by the name: \""+f+'"'}else{d.cache[f]=g;e[f]=function(j,i){j=j||{};var h=b.to_html(d.cache[f],j,d.partials);return i?h:c(h)}}}};this.addPartial=function(f,g){if(d.partials.hasOwnProperty(f)){throw'You\'ve already got a partial with the name: " + name + "'}else{d.partials[f]=g}};this.grabTemplates=function(){c('script[type="text/html"]').each(function(f){f=(typeof f=="number")?c(this):c(f);var g=f.attr("id"),i=f.html().trim(),h=(f.attr("class")&&f.attr("class").toLowerCase()==="partial");if(h){e.addPartial(g,i)}else{e.addTemplate(g,i)}f.remove()})};this.showAll=function(){return{templates:c.extend({},d.cache),partials:c.extend({},d.partials)}};this.clearAll=function(){for(var f in d.cache){delete e[f]}d.cache={};d.partials={}};this.refresh=function(){this.clearAll();this.grabTemplates()}}window.ich=new a();c(function(){ich.grabTemplates()})})(this.jQuery||this.Zepto);