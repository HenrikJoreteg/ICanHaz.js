...
c,b){if(!this.includes("#",a)&&!this.includes("^",a))return a;var d=this;return a.replace(RegExp(this.otag+"(\\^|\\#)\\s*(.+)\\s*"+this.ctag+"\n*([\\s\\S]+?)"+this.otag+"\\/\\s*\\2\\s*"+this.ctag+"\\s*","mg"),function(a,e,j,f){a=d.find(j,c);if("^"==e)return!a||d.is_array(a)&&0===a.length?d.render(f,c,b,!0):"";if("#"==e)return d.is_array(a)?d.map(a,function(a){return d.render(f,d.create_context(a),b,!0)}).join(""):d.is_object(a)?d.render(f,d.create_context(a),b,!0):"function"===typeof a?a.call(c,f,
function(a){return d.render(a,c,b,!0)}):a?d.render(f,c,b,!0):""})},render_tags:function(a,c,b,d){for(var e=this,h=function(){return RegExp(e.otag+"(=|!|>|\\{|%)?([^\\/#\\^]+?)\\1?"+e.ctag+"+","g")},j=h(),f=function(a,d,f){switch(d){case "!":return"";case "=":return e.set_delimiters(f),j=h(),"";case ">":return e.render_partial(f,c,b);case "{":return e.find(f,c);default:return e.escape(e.find(f,c))}},a=a.split("\n"),i=0;i<a.length;i++)a[i]=a[i].replace(j,f,this),d||this.send(a[i]);if(d)return a.join("\n")},
