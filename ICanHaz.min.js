...
$:window?window.jQuery||window.Zepto||null:null,addTemplate:function(a,b){console.log(a,b);if(f[a])throw"Invalid name: "+a+".";if(f.templates[a])throw'Template " + name + " exists';f.templates[a]=b;f[a]=function(c,d){c=c||{};var e=m.to_html(f.templates[a],c,f.templates);return f.$&&!d?f.$(e):e}},clearAll:function(){for(var a in f.templates)delete f[a];f.templates={}},refresh:function(){f.clearAll();f.grabTemplates()},grabTemplates:function(){var a,b=document.scripts,c=b.length,d,e=[];for(a=0;a<c;a++){d=
b[a];console.log("looped",d.id);if(d&&d.innerText&&d.id&&(d.type==="text/html"||d.type==="text/x-icanhaz")){f.addTemplate(d.id,"".trim?d.innerText.trim():s.replace(/^\s+/,"").replace(/\s+$/,""));e.unshift(d)}}a=0;for(c=e.length;a<c;a++)e[a].parentElement.removeChild(e[a])}};if(typeof require!=="undefined")module.exports=f;else window.ich=f;if(document)f.$?f.$(function(){f.grabTemplates()}):document.addEventListener("DOMContentLoaded",function(){f.grabTemplates()})})()})(window.jQuery||window.Zepto);
